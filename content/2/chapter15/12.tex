...
\begin{notice}注意占位符类型和占位符类类型之间的区别，占位符类型是auto或decltype(auto)，可以解析为任何类型，而占位符类类型是模板名称，只能解析为指定模板的实例的类类型。
\end{notice}
...
\begin{notice}与普通函数模板推导一样，也可以使用SFINAE(若在引导类型中替换推导出的参数失败)。这个简单的例子中，情况并非如此。
\end{notice}
...
\begin{notice}第16章介绍了以各种方式“特化”类模板的能力。此类特化不参与类模板参数推导。
\end{notice}
