...
\begin{notice}没有将该函数命名为size()，避免与C++标准库的命名冲突，C++17引入了标准函数模板std::size()。
\end{notice}
...
\begin{notice}实践中，保底候选函数通常会提供更有用的默认值、抛出异常或包含静态断言输出更有用的错误消息。
\end{notice}
...
\begin{notice}由于删除了类线程的复制构造函数，这也禁止了复制。
\end{notice}
