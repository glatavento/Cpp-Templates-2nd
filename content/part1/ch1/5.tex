...
若模板可以生成匹配更好的函数实例，则选择模板。这在 \inlcpp{max()} 的第二次和第三次调用中得到了验证：
...
\filename{basics/maxdefault4.hpp}
\begin{cpp}
...
就会导致运行时错误，因为对于C字符，\inlcpp{max(a,b)} 创建了一个新临时变量，并通过引用返回。但这个临时值越过返回语句完成就会销毁，留给 \inlcpp{max(a,b)} 一个悬空引用。不幸的是，这个错误非常微妙，可能不必现。

\begin{notice}
一般来说，符合标准的编译器可以编译这段代码。
\end{notice}

与此相反，\inlcpp{main()} 中对 \inlcpp{main()} 的第一次调用不会出现相同的问题。虽为参数（7、42和68）创建了临时变量，但这些临时变量在 \inlcpp{main()} 中创建，\inlcpp{main()} 中会一直存在，直到程序执行完毕。
