...
\begin{notice}EnableIf也可以放在默认的模板参数中，这比放在结果类型中更有优势。参见第20.3.2节关于EnableIf位置的讨论。
\end{notice}
...
\begin{notice}算法特化只用于在计算时间或资源使用方面提供效率增益。然而，一些算法的特化也提供了更多的功能，例如(在本例中)按顺序向后移动的能力。
\end{notice}
...
\begin{notice}虽然转换函数模板确实有返回类型，但转换到该类型的模板参数必须可推导(参见第15章)，这样转换函数模板才能正常工作。
\end{notice}
